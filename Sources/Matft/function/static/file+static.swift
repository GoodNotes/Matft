...
fileprivate class _TxtParser{
    ...
    func load() -> [String]?{
        guard let contentData = try? Data(contentsOf: self.url) else{
            print("cannot load. You may pass invalid path or url")
            return nil
        }
        ...
        if self.removeBlank{// remove white space. e.g.) ,3 4, -> ,34,
            contentString.removeAll(where: { $0 == " " })
        }

        var lines = contentString.split(whereSeparator: \.isNewline).map(String.init)
